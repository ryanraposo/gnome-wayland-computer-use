...
candidates=("$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd)/scripts/teardown.sh" "$HOME/.agents/skills/gnome-wayland-computer-use/scripts/teardown.sh" "${HERMES_HOME:-$HOME/.hermes}/skills/computer-use/scripts/teardown.sh")
