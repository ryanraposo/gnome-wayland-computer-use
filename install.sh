...
# ── Main ────────────────────────────────────────────────────────────────
header "gnome-wayland-computer-use"
...
[ "$SESSION" != "wayland" ] && { fail "Session is '$SESSION' — need Wayland"; exit 1; } || ok "Display server: Wayland"
[[ "$DESKTOP" != *"GNOME"* ]] && { fail "Desktop is '$DESKTOP' — need GNOME"; exit 1; } || ok "Desktop: $DESKTOP"
command -v gsettings &>/dev/null && ok "gsettings available" || { fail "gsettings not found"; exit 1; }
...
if ! $FORCE && [ -f "$EXT_DIR/extension.js" ] && command -v gnome-extensions &>/dev/null; then
  ...
  if echo "$STATE" | grep -qi active; then
    header "Already installed and active"
    echo "  Nothing to do."
    echo ""
    echo "  ${B}Test:${N}  gdbus introspect --session --dest org.cua.WinRects --object-path /org/cua/WinRects"
    echo ""
    exit 0
  fi
  info "Extension installed but $STATE — need log out/in or --force"
  $FORCE || exit 0
fi
...
echo ""
echo -e "  ${B}┌──────────────────────────────────────────────────────────────┐${N}"
echo -e "  ${B}│${N}  ${G}Install complete${N}                                              ${B}│${N}"
echo -e "  ${B}│${N}                                                              ${B}│${N}"
echo -e "  ${B}│${N}  ${G}✓${N} Skill installed → ~/.agents/skills/gnome-wayland-computer-use/SKILL.md ${B}│${N}"
echo -e "  ${B}│${N}                                                              ${B}│${N}"
echo -e "  ${B}│${N}  ${Y}Next step:${N}  Log out and log back in                               ${B}│${N}"
echo -e "  ${B}│${N}                                                              ${B}│${N}"
echo -e "  ${B}│${N}  ${Y}Verify:${N}     gnome-extensions info winrects@cua                ${B}│${N}"
echo -e "  ${B}│${N}               gdbus introspect --session \\                  ${B}│${N}"
echo -e "  ${B}│${N}                 --dest org.cua.WinRects                     ${B}│${N}"
echo -e "  ${B}│${N}                 --object-path /org/cua/WinRects             ${B}│${N}"
echo -e "  ${B}└──────────────────────────────────────────────────────────────┘${N}"
echo ""
