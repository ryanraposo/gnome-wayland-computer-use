...
for f in SKILL.md runtimes/openai/SKILL.md README.md WORLDLINE.md DETERMINISM.md CAPABILITIES.md; do
    grep -qi 'Cua' "$ROOT/$f" || fail "$f lost Cua authority"
    grep -qi 'WORLDLINE' "$ROOT/$f" || fail "$f lost WORLDLINE architecture"
done
pass "public architecture is apprehensible from every entry point"

for f in SKILL.md runtimes/openai/SKILL.md README.md DETERMINISM.md; do
...
grep -qi 'valid until invalidated' "$ROOT/WORLDLINE.md" || fail "WORLDLINE invalidation rule missing"
grep -qi 'postcondition' "$ROOT/DETERMINISM.md" || fail "determinism lost predicate rule"
grep -qi 'WORLDLINE.*transient' "$ROOT/GWCU.md" || fail ".gwcu/WORLDLINE lifetime boundary missing"
pass "state lifetimes and postconditions are explicit"
