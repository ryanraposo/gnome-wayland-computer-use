...
grep -q '^## Workflow Contract$' "$ROOT/SKILL.md" || fail "Hermes skill owns the workflow"
grep -q '^## Execution State Machine$' "$ROOT/SKILL.md" || fail "Hermes skill declares its state machine"
grep -q '^## Latency-First Interaction$' "$ROOT/SKILL.md" || fail "Hermes skill owns end-to-end latency"
grep -q '^## Pixel-Only Surfaces$' "$ROOT/SKILL.md" || fail "Hermes skill owns inaccessible-surface recovery"
grep -q '^## Workflow Contract$' "$ROOT/runtimes/openai/SKILL.md" || fail "portable skill owns the workflow"
grep -q '^## Latency-First Interaction$' "$ROOT/runtimes/openai/SKILL.md" || fail "portable skill owns end-to-end latency"
grep -q '^## Pixel-Only Surfaces$' "$ROOT/runtimes/openai/SKILL.md" || fail "portable skill owns inaccessible-surface recovery"
...
[ "$version" = 2.3.0 ] || fail "native-capture release version is 2.3.0"
