...
grep -q 'doctor --json' "$ROOT/install.sh" || fail "installer does not verify with Cua doctor"
...
assert d['schema']=='gwcu.diagnose.v2'
assert d['ok'] is False
assert d['code']=='wrong_session'
assert 'observation' in d and 'cua' in d
PY
