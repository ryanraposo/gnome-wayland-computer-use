...
MAX_REQUEST=16384; DEFAULT_IDLE=120; DEFAULT_SESSION_TIMEOUT=40
Gio=GLib=Gst=GstVideo=GdkPixbuf=None

class E(Exception):
    def __init__(self, code, exit_code=40, retryable=True, terminal=False, detail=None):
        super().__init__(detail or code); self.code=code; self.exit_code=exit_code; self.retryable=retryable; self.terminal=terminal; self.detail=detail or code
...
def env(ok, code, result=None, *, retryable=None, terminal=None, detail=None, next=None, timing_ms=None):
    ...
    d["next"]=next; return d

def runtime_dir():return pathlib.Path(os.environ.get("XDG_RUNTIME_DIR",f"/run/user/{os.getuid()}"))/APP
def state_dir():return pathlib.Path(os.environ.get("XDG_STATE_HOME",str(pathlib.Path.home()/".local/state")))/APP
...
def gi():
    ...
    try:
        import gi as _gi
        for n,v in (("Gio","2.0"),("Gst","1.0"),("GstVideo","1.0"),("GdkPixbuf","2.0")):_gi.require_version(n,v)
        from gi.repository import Gio as A,GLib as B,Gst as C,GstVideo as D,GdkPixbuf as F
    except Exception as ex:raise E("gi_unavailable",30,False,detail=str(ex))
...
class Cast:
    ...
    def proxy(self):
        ...
        self.portal=Gio.DBusProxy.new_sync(self.bus,Gio.DBusProxyFlags.NONE,None,"org.freedesktop.portal.Desktop","/org/freedesktop/portal/desktop","org.freedesktop.portal.ScreenCast",None)
    ...
    def close_obj(self,path,iface):
        ...
        try:self.bus.call_sync("org.freedesktop.portal.Desktop",path,iface,"Close",None,None,Gio.DBusCallFlags.NONE,1000,None)
        except Exception:pass
    def request(self,method,signature,values,timeout=20):
        self.proxy();sender=self.bus.get_unique_name().lstrip(":").replace(".","_");token="gwcu_"+uuid.uuid4().hex;expected=f"/org/freedesktop/portal/desktop/request/{sender}/{token}";loop=GLib.MainLoop();response={}
        def got(_c,_s,_p,_i,_n,params):response["v"]=params.unpack();loop.quit()
        sub=self.bus.signal_subscribe("org.freedesktop.portal.Desktop","org.freedesktop.portal.Request","Response",expected,None,Gio.DBusSignalFlags.NONE,got);actual=expected
        try:
            values[-1]["handle_token"]=GLib.Variant("s",token);reply=self.portal.call_sync(method,GLib.Variant(signature,tuple(values)),Gio.DBusCallFlags.NONE,3000,None);unpacked=reply.unpack() if reply else ()
            if unpacked and isinstance(unpacked[0],str):actual=unpacked[0]
            if actual!=expected:
                self.bus.signal_unsubscribe(sub);sub=self.bus.signal_subscribe("org.freedesktop.portal.Desktop","org.freedesktop.portal.Request","Response",actual,None,Gio.DBusSignalFlags.NONE,got)
            timer=GLib.timeout_add(int(timeout*1000),lambda:(loop.quit(),False)[1]);loop.run()
        ...
        if code:raise E("portal_cancelled" if code==1 else "portal_interaction_ended",20,False,True,f"{method} response {code}")
        return results
    def token(self,new=None):
        p=state_dir()/"screencast-restore-token"
        if new:
            private_dir(p.parent);tmp=p.with_name(f".{p.name}.{os.getpid()}.tmp");tmp.write_text(new,encoding="utf-8");tmp.chmod(0o600);tmp.replace(p);return None
    ...
    def on_sample(self,sink):
        ...
        with self.cond:self.sample=s;self.frame_ns=time.monotonic_ns();self.cond.notify_all()
        return Gst.FlowReturn.OK
    def start_pipeline(self,fd,node,props):
        pipe=Gst.Pipeline.new("gwcu");src=Gst.ElementFactory.make("pipewiresrc","src");conv=Gst.ElementFactory.make("videoconvert","conv");filt=Gst.ElementFactory.make("capsfilter","rgb");sink=Gst.ElementFactory.make("appsink","sink")
        if not all((pipe,src,conv,filt,sink)):raise E("gstreamer_elements_missing",30,False)
        src.set_property("fd",fd);serial=props.get("pipewire-serial")
        if self.version>=6 and serial is not None and src.find_property("target-object"):src.set_property("target-object",str(serial))
        ...
        if pipe.get_state(3*Gst.SECOND)[0]==Gst.StateChangeReturn.FAILURE:pipe.set_state(Gst.State.NULL);raise E("pipewire_stream_failed")
        self.pipeline=pipe
    def ensure(self,session_timeout_s=DEFAULT_SESSION_TIMEOUT):
        if self.active:return
        self.proxy();session_timeout_s=max(10,min(float(session_timeout_s),120));short=min(10,session_timeout_s);start=max(10,session_timeout_s-short)
        try:
            c=self.request("CreateSession","(a{sv})",[{"session_handle_token":GLib.Variant("s","gwcu_"+uuid.uuid4().hex)}],short);self.session=c["session_handle"]
            opts={"types":GLib.Variant("u",1),"multiple":GLib.Variant("b",False)};modes=self.portal.get_cached_property("AvailableCursorModes")
            if modes is not None and int(modes.unpack())&2:opts["cursor_mode"]=GLib.Variant("u",2)
            if self.version>=4:
                opts["persist_mode"]=GLib.Variant("u",2);old=self.token()
                if old:opts["restore_token"]=GLib.Variant("s",old)
            self.request("SelectSources","(oa{sv})",[self.session,opts],short);started=self.request("Start","(osa{sv})",[self.session,"",{}],start);streams=started.get("streams",[])
            if not streams:raise E("portal_no_streams")
            node,props=streams[0];self.token(started.get("restore_token"));reply,fds=self.portal.call_with_unix_fd_list_sync("OpenPipeWireRemote",GLib.Variant("(oa{sv})",(self.session,{})),Gio.DBusCallFlags.NONE,3000,None,None);self.pwfd=fds.get(reply.unpack()[0]);self.start_pipeline(self.pwfd,node,props or {});self.generation+=1
...
def listener():
    ...
    if n>=1 and pid==os.getpid():s=socket.fromfd(3,socket.AF_UNIX,socket.SOCK_STREAM);s.setblocking(True);return s
...
def handle(cast,q):
    ...
    if op=="capture":
        ...
        return cast.capture(fresh,min(max(int(q.get("timeout_ms",1500)),50),10000),min(max(int(q.get("session_timeout_ms",40000)),10000),120000))
...
def client(op,fresh="next",timeout_ms=1500,session_timeout_ms=40000):
    q={"v":1,"op":op}
    if op=="capture":q|={"fresh":fresh,"timeout_ms":timeout_ms,"session_timeout_ms":session_timeout_ms}
    # Session/consent establishment can legitimately take tens of seconds. Keep
    # that budget independent from the post-session fresh-frame budget.
    wire_timeout=.5+(session_timeout_ms/1000 if op=="capture" else 0)+(timeout_ms/1000 if op=="capture" else 2)
    s=socket.socket(socket.AF_UNIX,socket.SOCK_STREAM);s.settimeout(max(2,min(125,wire_timeout)))
    try:
        ...
        r=json.loads(raw.split(b"\n",1)[0]);rc=int(r.pop("_exit",0 if r.get("ok") else 40));print(json.dumps(r,separators=(",",":")));return rc
    except (FileNotFoundError,ConnectionRefusedError):print(json.dumps(env(False,"broker_unavailable",retryable=True,terminal=False,next={"action":"start_observer_socket"}),separators=(",",":")));return 30
    except E as ex:print(json.dumps(env(False,ex.code,retryable=ex.retryable,terminal=ex.terminal,detail=ex.detail),separators=(",",":")));return ex.exit_code
    except Exception as ex:print(json.dumps(env(False,"broker_protocol_error",retryable=True,terminal=False,detail=str(ex)),separators=(",",":")));return 40
    finally:s.close()

def selftest():
    p=runtime_dir();private_dir(p);assert stat.S_IMODE(p.stat().st_mode)&0o077==0
    print(json.dumps({"schema":"gwcu.observer.selftest.v1","ok":True,"code":"ok","checks":["private_runtime_dir","stable_envelope","lazy_gi_import","separate_session_frame_timeouts"]},separators=(",",":")));return 0

def main():
    p=argparse.ArgumentParser();sub=p.add_subparsers(dest="cmd",required=True);sub.add_parser("serve");sub.add_parser("self-test");c=sub.add_parser("client");c.add_argument("op",choices=["capture","status","close"]);c.add_argument("--fresh",choices=["next","latest"],default="next");c.add_argument("--timeout-ms",type=int,default=1500);c.add_argument("--session-timeout-ms",type=int,default=int(os.environ.get("GWCU_OBSERVER_SESSION_TIMEOUT_MS","40000")));a=p.parse_args()
    if a.cmd=="serve":return serve()
    if a.cmd=="self-test":return selftest()
    return client(a.op,a.fresh,a.timeout_ms,a.session_timeout_ms)
